...
#					print (((int(score) * similarity) / 100 <= int(alignmentLength)))
					if (int(sequenceLength) * percentage) / 100 <= int(alignmentLength) and (int(alignmentLength) * similarity) / 100 <= int(score): #and (int(sequenceLengthFirst) * percentage) / 100 <= int(alignmentLengthFirst):
						keep = True
				else:
#					line = inputLines[i].split()
